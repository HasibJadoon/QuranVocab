...
export const onRequestPost: PagesFunction<Env> = async (ctx) => {
  try {
    ...
    const lessonType = asString(body.lesson_type ?? body.lessonType);
    if (!lessonType) return badRequest('lesson_type is required');
    ...
    await ctx.env.DB.prepare(
      `
        ...
        ) VALUES (?1, NULL, ?2, ?3, 'draft', ?4, 1, ?5)
      `
    )
      .bind(draftId, user.id, lessonType, activeStep, JSON.stringify(draftJson))
      .run();
