...
import { composeCaptureBody } from '../utils/capture-meta.util';
...
export class CaptureNotesService {
  ...
  create(args: {
    ...
    return this.http.post<{ ok: boolean; note: CaptureNote }>(`${this.apiRoot}/notes/capture`, {
      ...
      body_md: composeCaptureBody(args.meta, args.text),
      meta: args.meta,
      text: args.text,
    }).pipe(
  ...
  archive(captureId: string): Observable<CaptureNote> {
    return this.http.post<{ ok: boolean; note: CaptureNote }>(
      `${this.apiRoot}/notes/capture/${encodeURIComponent(captureId)}/archive`,
      {}
