...
@Component({
  selector: 'mcit-full-country-field',
  templateUrl: './full-country-field.component.html',
  styleUrls: ['./full-country-field.component.scss'],
  providers: [
    McitMeaningPipe,
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => McitFullCountryFieldComponent),
      multi: true
    }
  ]
})
export class McitFullCountryFieldComponent implements OnInit, OnDestroy, ControlValueAccessor {
  ...
  writeValue(value: any) {
    if (value) {
      this.country = value;
      this.countryFieldForm.controls['country'].setValue(this.meaningPipe.transform(value.names), { emitEvent: false });
    } else {
      this.country = null;
