...
import { AppViewJsonModalComponent } from '../app-view-json-modal/app-view-json-modal.component';

export interface CrudTableColumn {
  key: string;
  label?: string;
  type?: 'text' | 'badge' | 'json';
  className?: string;
...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, AppViewJsonModalComponent],
...
export class AppCrudTableComponent {
  ...
  jsonModalOpen = false;
  jsonModalTitle = 'JSON Viewer';
  jsonModalValue: unknown;
  ...
  hasJsonValue(column: CrudTableColumn, row: any) {
  ...
  openJsonViewer(column: CrudTableColumn, row: any) {
    this.jsonModalTitle = column.jsonTitle || column.label || this.humanize(column.key);
    this.jsonModalValue = this.resolveCellValue(column, row);
    this.jsonModalOpen = true;
  }

  closeJsonViewer() {
    this.jsonModalOpen = false;
    this.jsonModalValue = undefined;
  }

  jsonColumnIcon() {
    return this.iconLibrary['view'];
  }
