...
import { metersToPixelsAtMaxZoom } from '../../../../helpers/map.helper';
import { ICategoryGeoDistanceConfig } from '../../../facet-options';
...
@Component({
  ...
  templateUrl: './category-geo-distance-modal.component.html',
  styleUrls: ['./category-geo-distance-modal.component.scss'],
  providers: [McitDistancePipe]
})
export class McitCategoryGeoDistanceModalComponent implements OnInit {
  name: string;
  marker: MapboxGl.Marker;

  constructor(
    private dialogRef: McitDialogRef<McitCategoryGeoDistanceModalComponent>,
    @Inject(MCIT_DIALOG_DATA)
    private data: {
      boundaries: ICategoryGeoDistanceConfig['geoDistance']['boundaries'];
  ...
  doLoadedMap(mapContainer: MapboxGl.Map): void {
    ...
    const list = this.data.boundaries.filter((b) => b.max != null).reverse();

    for (const b of list) {
      mapContainer.addSource(`source_${b.value}`, {
        type: 'geojson',
        data: {
          type: 'Feature',
          geometry: {
      ...
      mapContainer.addLayer({
        ...
        paint: {
          'circle-radius': {
            stops: [
              [0, 0],
              [20, metersToPixelsAtMaxZoom(b.max * 1000, 0)]
            ],
            base: 2
          },
          'circle-color': '#000000',
    ...
    for (const b of list) {
      mapContainer.addLayer({
        ...
        minzoom: 1,
        layout: {
          'text-field': this.distancePipe.transform(b.max * 1000),
        ...
        paint: {
          ...
          'text-translate': {
            stops: [
              [0, [0, 0]],
              [20, [0, metersToPixelsAtMaxZoom(b.max * 1000, 0)]]
            ],
            base: 2
          } as any,
  ...
  private update(mapContainer: MapboxGl.Map, lng: number, lat: number, searchName: boolean): void {
    ...
    for (const b of this.data.boundaries) {
      if (b.max != null) {
        (mapContainer.getSource(`source_${b.value}`) as GeoJSONSource).setData({
          type: 'Feature',
          geometry: {
            type: 'Point',
            coordinates: [lng, lat]
        ...
        mapContainer.setLayoutProperty(`circle_${b.value}`, 'visibility', 'visible');
        mapContainer.setPaintProperty(`circle_${b.value}`, 'circle-radius', {
          stops: [
            [0, 0],
            [20, metersToPixelsAtMaxZoom(b.max * 1000, lat)]
          ],
          base: 2
        });

        mapContainer.setLayoutProperty(`label_${b.value}`, 'visibility', 'visible');
        mapContainer.setPaintProperty(`label_${b.value}`, 'text-translate', {
          stops: [
            [0, [0, 0]],
            [20, [0, metersToPixelsAtMaxZoom(b.max * 1000, lat)]]
          ],
          base: 2
        });
      }
